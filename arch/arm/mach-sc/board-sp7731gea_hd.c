...
static void __init sc8830_pmu_late_init(void)
{
	u32 reg = 0;

	reg = __raw_readl(REG_MM_AHB_GEN_CKG_CFG);
	reg &= ~(BIT_MM_MTX_AXI_CKG_EN | BIT_MM_AXI_CKG_EN);
	__raw_writel(reg, REG_MM_AHB_GEN_CKG_CFG);

	reg = __raw_readl(REG_AON_APB_APB_EB0);
	reg &= ~(BIT_MM_EB | BIT_GPU_EB);
	__raw_writel(reg, REG_AON_APB_APB_EB0);

	reg = __raw_readl(REG_PMU_APB_PD_GPU_TOP_CFG);
	reg |= BIT_PD_GPU_TOP_FORCE_SHUTDOWN;
	__raw_writel(reg, REG_PMU_APB_PD_GPU_TOP_CFG);

	reg = __raw_readl(REG_PMU_APB_PD_MM_TOP_CFG);
	reg |= BIT_PD_MM_TOP_FORCE_SHUTDOWN;
	__raw_writel(reg, REG_PMU_APB_PD_MM_TOP_CFG);
}
