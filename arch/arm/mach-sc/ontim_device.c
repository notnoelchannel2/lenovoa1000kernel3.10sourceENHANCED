...
/* A1000: middle/right hardkeys were swapped. The panel reports the three
 * capacitive keys as plain touches at y=860 with x=400 (left), x=200 (middle)
 * and x=300 (right) - measured with getevent - so the middle key was landing on
 * the BACK entry and the right key on the HOMEPAGE one. Swap the two key codes;
 * the coordinates match what the controller actually sends and must stay. */
...
static struct mc3xxx_platform_data mc3xxx_plat_data = {
	.power_on=sensor_set_power,
	.current_placement=MC3XXX_TOP_LEFT_DOWN , /* A1000: подтверждено на устройстве, не менять вслепую */
};
#endif
