...
//	struct semaphore   overlay_lock;
	...
	struct completion first_frame_completion;
};
...
extern u8 sprdfb_esd_enable; //add by liuwei
#endif
...
int wait_first_frame_complete_sleepms = 70;
void wait_first_frame_complete()
{
	wait_for_completion_timeout(&dispc_ctx.first_frame_completion, msecs_to_jiffies(300));
	msleep(wait_first_frame_complete_sleepms); //wait first frame
}
EXPORT_SYMBOL(wait_first_frame_complete);
...
static void dispc_run(struct sprdfb_device *dev)
{
	...
	if(SPRDFB_PANEL_IF_DPI == dev->panel_if_type){
		...
		if(dispc_ctx.is_first_frame){
			...
			printk(">>>> first frame, inform turn on backlight\n");
			complete(&dispc_ctx.first_frame_completion);
		} else {
...
static int32_t sprdfb_dispc_early_init(struct sprdfb_device *dev)
{
	...
	init_completion(&dispc_ctx.first_frame_completion);
