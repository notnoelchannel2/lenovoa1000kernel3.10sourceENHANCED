...
static int sprd_backlight_set_val(char v) //wanggang
{
    unsigned int addr = SPRD_ADISLAVE_BASE + SPRD_FLASH_OFST ;

    sci_adi_write( SPRD_ADISLAVE_BASE + SPRD_FLASH_OFST, SPRD_FLASH_CTRL_BIT | v, SPRD_FLASH_CTRL_BIT | 0xf) ;
    //sci_adi_set(SPRD_ADISLAVE_BASE + SPRD_FLASH_OFST, SPRD_FLASH_CTRL_BIT | v); /*0xf = 470ma*/
    return 0 ;
}
static int sprd_backlight_close(void)
{
	sci_adi_clr(SPRD_ADISLAVE_BASE + SPRD_FLASH_OFST, SPRD_FLASH_CTRL_BIT | 0xF);
    return 0 ;
}
#endif
static int sprd_bl_pwm_update_status(struct backlight_device *bldev)
{
	...
	if ((bldev->props.state & (BL_CORE_SUSPENDED | BL_CORE_FBBLANK)) ||
		...
		/* disable backlight */
#ifdef CONFIG_ONTIM_LCD_BACKLIGHT_BY_FLASH_DRV
        sprd_backlight_close(); //wanggang
#else
		pwm_write(sprdbl.pwm_index, 0, PWM_PRESCALE);
		bl_pwm_clk_en(0);
#endif
		printk("[kernel] [pwm] disabled\n");
	} else {
...
extern void wait_first_frame_complete();
