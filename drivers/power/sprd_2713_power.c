...
enum sprdbat_event {
	...
	/*wdm---*/
	SPRDBAT_CHG_OVERHEATLEVE_STOP_E,
        SPRDBAT_CHG_OVERHEATLEVE_RESET_E,
	/*---*/
};
...
extern bool charger_probe_ok;//bit5 add by liuwei
extern bool pmu_probe_ok;//bit5 add by liuwei
...
#endif
/*wdm ---*/
int sprdbat_get_temp()
{
        return sprdbat_data->bat_info.cur_temp;
}
/*---*/
...
static struct device_attribute sprd_caliberate[] = {
	...
	/*wdm---*/
	SPRDBAT_CALIBERATE_ATTR(temp_over_h),
        SPRDBAT_CALIBERATE_ATTR(temp_over_l),
	SPRDBAT_CALIBERATE_ATTR(temp_protection_switch),
	/*---*/
};
...
enum temp_h_change {
        ...
        TEMP_BACK_45_LEVEL,
        TEMP_OVER_45_LEVEL,
        TEMP_H_BACK_4100_OCV,
        TEMP_H_OVER_4100_OCV,
        TEMP_BACK_0_LEVEL,
        TEMP_OVER_0_LEVEL,
        TEMP_DEFAULT_VAL,
};
/*---*/
/*wdm---*/
static int sprd_temp_detect_on = 1;
//static int stop_flag_set = 0;
//static int usb_now_current_level = 0;
static int temp_voltage_reset = 0;
/*---*/
static ssize_t sprdbat_store_caliberate(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	case HW_SWITCH_POINT:
		if (!sprdbat_cccv_cal_from_chip) {
			...
			sprdchg_set_cccvpoint(sprdbat_data->bat_info.cccv_point,sprdbat_data->pdata->chg_end_vol_pure);
	...
	/*wdm---*/
	case TEMP_OVER_H:
                printk("+++++sprdbat_data->bat_info.temp_over_h = %d\n",sprdbat_data->bat_info.temp_over_h);
                sprdbat_data->bat_info.temp_over_h = set_value;
                break;
        case TEMP_OVER_L:
                printk("+++++sprdbat_data->bat_info.temp_over_l = %d\n",sprdbat_data->bat_info.temp_over_l);
                sprdbat_data->bat_info.temp_over_l = set_value;
                break;
	case TEMP_PROTECTION_SWITCH:
                printk("trun off temp protection\n");
                sprd_temp_detect_on = set_value;
                break;
	/*---*/
	default:
		count = -EINVAL;
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	/*wdm---*/
	case TEMP_OVER_H:
                printk("update over hight\n");
                adc_value = sprdbat_data->bat_info.temp_over_h;
                i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", adc_value);
                break;
        case TEMP_OVER_L:
                printk("update over low\n");
                adc_value = sprdbat_data->bat_info.temp_over_l;
                i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", adc_value);
                break;
	case TEMP_PROTECTION_SWITCH:
                printk("switch on test\n");
                i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n", sprd_temp_detect_on);
                break;
	/*---*/
	default:
...
static void sprdbat_info_init(struct sprdbat_drivier_data *data)
{
	...
	/*wdm---*/
	data->bat_info.temp_over_l = 0;
        data->bat_info.temp_over_h = 0;
	/*---*/
...
static int sprdbat_start_charge(void)
{
	...
	/*wdm---*/
	if(temp_voltage_reset == 1){
                temp_voltage_reset = 0;
                sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_high_temp_area);
        }
        else
	/*---*/
...
static inline void _sprdbat_set_stopflags(uint32_t flag)
{
	...
	if (sprdbat_data->bat_info.module_state == POWER_SUPPLY_STATUS_CHARGING) {
		...
		/*wdm---*/
		} else if (flag == SPRDBAT_CHG_OVERHEAT_LEVE_BIT) {
		/*---*/
		} else {
			sprdbat_data->bat_info.module_state =
...
static void sprdbat_change_module_state(uint32_t event)
{
	...
	case SPRDBAT_CHG_TIMEOUT_RESTART_E:
		...
		break;
	/*wdm---*/
	case SPRDBAT_CHG_OVERHEATLEVE_STOP_E:
                        _sprdbat_set_stopflags(SPRDBAT_CHG_OVERHEAT_LEVE_BIT);
                break;
        case SPRDBAT_CHG_OVERHEATLEVE_RESET_E:
                        _sprdbat_clear_stopflags(SPRDBAT_CHG_OVERHEAT_LEVE_BIT);
                break;
	/*---*/
	default:
		BUG_ON(1);
...
static int plugout_callback(int usb_cable, void *data)
{
	...
	sprdbat_change_module_state(SPRDBAT_ADP_PLUGOUT_E);
	/*wdm---*/
	sprdbat_data->bat_info.temp_over_h  = TEMP_DEFAULT_VAL;
	/*---*/
#ifndef SPRDBAT_TWO_CHARGE_CHANNEL
	wake_lock_timeout(&(sprdbat_data->charger_plug_out_lock),
			  SPRDBAT_PLUG_WAKELOCK_TIME_SEC * HZ);
...
#ifndef SPRDBAT_TWO_CHARGE_CHANNEL
#ifndef CONFIG_ARCH_SCX35L64    //mingwei TODO
...
static int sprdbat_adjust_cccvpoint(uint32_t vbat_now)
{
	...
	if (vbat_now <= sprdbat_data->pdata->chg_end_vol_pure) {
		...
		sprdchg_set_cccvpoint(cv,sprdbat_data->pdata->chg_end_vol_pure);
	} else {
		cv = sprdchg_get_cccvpoint();
...
ocv:%d,current:%d,cap:%d,state:%d,auxbat:%d,temp:%d,rint:%d,ave:%d\n", cur_time.tv_sec, sprdbat_data->bat_info.vbat_vol,
...
static void sprdbat_update_capacty(void)
{
	...
	/*wdm---*/
	static uint32_t last_module_state = 0;
	/*---*/
	struct timespec cur_time;
	...
	sprdbat_last_query_time = cur_time.tv_sec;
	/*wdm---*/
	if(last_module_state != sprdbat_data->bat_info.module_state)
       {
                last_module_state = sprdbat_data->bat_info.module_state;
                flush_time =0;
                sprdbat_update_capacity_time = cur_time.tv_sec ;
       }
	/*---*/
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		...
		if(fgu_capacity > 90)
                	fgu_capacity -= 1;
		if ( (fgu_capacity <= sprdbat_data->bat_info.capacity) && (sprdbat_data->bat_info.chg_stop_flags != SPRDBAT_CHG_OVERHEAT_LEVE_BIT) ) {
		/*---*/
			fgu_capacity = sprdbat_data->bat_info.capacity;
		} else {
			if ((period_time < sprdbat_data->pdata->cap_one_per_time) || (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_OVERHEAT_LEVE_BIT)) {
				fgu_capacity =
		...
		if (100 == sprdbat_data->bat_info.capacity) {
			...
			if (fgu_capacity >= 100) {
				if(sprdbat_data->bat_info.chg_stop_flags != SPRDBAT_CHG_OVERHEAT_LEVE_BIT)
				fgu_capacity = 99;
				else
				fgu_capacity = 100;
			}
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		if (fgu_capacity >= sprdbat_data->bat_info.capacity) {
			...
			if(fgu_capacity <5)
                       {
                               if((sprdbat_data->bat_info.capacity - fgu_capacity) > 15)
                                        sprdbat_data->pdata->cap_one_per_time =10;
                               else if((sprdbat_data->bat_info.capacity - fgu_capacity) > 5)
                                        sprdbat_data->pdata->cap_one_per_time =20;
                               else
                                        sprdbat_data->pdata->cap_one_per_time =30;
                       }
                        else
                        {
                               if((sprdbat_data->bat_info.capacity - fgu_capacity) > 30)
                                        sprdbat_data->pdata->cap_one_per_time =10;
                                else
                               if((sprdbat_data->bat_info.capacity - fgu_capacity) > 15)
                                        sprdbat_data->pdata->cap_one_per_time =15;
                               else if((sprdbat_data->bat_info.capacity - fgu_capacity) > 5)
                                        sprdbat_data->pdata->cap_one_per_time =30;
                               else
                                        sprdbat_data->pdata->cap_one_per_time =45;
                        }
			/*---*/
...
static void sprdbat_battery_works(struct work_struct *work)
{
	...
	/*wdm---*/
	power_supply_changed(&sprdbat_data->battery);    //add for upload temp in time
	/*---*/
	mutex_unlock(&sprdbat_data->lock);
...
static void sprdbat_temp_monitor(void)
{
	...
	/*wdm++*/
	/*wdm---*/
	if(!sprd_temp_detect_on){
        printk("tempture detect protect has been closed!!!!\n");
        if((sprdbat_data->bat_info.chg_stop_flags & SPRDBAT_CHG_END_OTP_OVERHEAT_BIT)
                || (sprdbat_data->bat_info.chg_stop_flags & SPRDBAT_CHG_END_OTP_COLD_BIT)
		|| (sprdbat_data->bat_info.chg_stop_flags & SPRDBAT_CHG_OVERHEAT_LEVE_BIT))
        sprdbat_change_module_state(SPRDBAT_ADP_PLUGIN_E);
        return;
        }
	/*---*/
	...
	if (sprdbat_data->bat_info.
		...
		if (sprdbat_data->bat_info.cur_temp >
			...
			/*wdm---*/
			//temp_voltage_reset = 1;
                        //sprdbat_data->bat_info.temp_over_h= TEMP_BACK_0_LEVEL;
			/*---*/
		...
		if (sprdbat_data->bat_info.cur_temp <
			...
			/*wdm---*/
			//sprdbat_data->bat_info.temp_over_h = 0;
                        //temp_voltage_reset = 1;
			/*---*/
		...
		if (sprdbat_data->bat_info.cur_temp <
			...
			if (temp_trigger_cnt > SPRDBAT_TEMP_TRIGGER_TIMES) {
				if (sprdbat_data->bat_info.cur_temp <
				    sprdbat_data->pdata->otp_low_stop) {
					/*wdm---*/
					sprdbat_data->bat_info.temp_over_h = TEMP_OVER_0_LEVEL;
					/*---*/
					sprdbat_change_module_state
					    (SPRDBAT_OTP_COLD_STOP_E);
				} else {
					/*wdm---*/
					sprdbat_data->bat_info.temp_over_h = 1;
					/*---*/
					sprdbat_change_module_state
					    (SPRDBAT_OTP_OVERHEAT_STOP_E);
				}
		...
		/*wdm---*/
		printk(         "&&&&&&&&&&&&& \n,   \
                                        temp momtier sprdbat_data->pdata->low_temp_area_h = %d\n,  \
                                        sprdbat_data->pdata->low_temp_area_l = %d\n,   \
                                        sprdbat_data->bat_info.cur_temp = %d\n,   \
                                        temp_modify_flag = %d\n, \
                                        sprdbat_data->bat_info.vbat_ocv1 = %d\n, \
                                        sprdbat_data->bat_info.chg_stop_flags = 0x%x\n, \
                                        ----------------------\n",
                                        sprdbat_data->pdata->low_temp_area_h,
                                        sprdbat_data->pdata->low_temp_area_l,
                                        sprdbat_data->bat_info.cur_temp,
                                        temp_modify_flag,
                                        sprdbat_data->bat_info.vbat_ocv,
                                        sprdbat_data->bat_info.chg_stop_flags);
		...
		if(call_state == 1)
		{
			if(temp_modify_flag != 0x10){
				printk("in call state and change current\n");
				sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_call);
			}
                        temp_modify_flag = 0x10;
		}
		else
		{
		/*wdm++*/
                if(sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT ||
                        sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_OVERHEAT_LEVE_BIT){
                        if((sprdbat_data->bat_info.cur_temp <= sprdbat_data->pdata->low_temp_area_h) &&
                        (sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->low_temp_area_l)){
                                if(temp_modify_flag != 0x01){
                                        printk("battery temp in a low area\n");
                                        sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_low_temp_area);
                                }
                                temp_modify_flag = 0x01;
                        }
			else if((sprdbat_data->bat_info.cur_temp <= sprdbat_data->pdata->high_temp_area_l_cur_30) &&
                        (sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->low_temp_area_h)){
                                if(temp_modify_flag != 0x02){
                                        printk("battery temp in a low 30 area\n");
					sprdchg_set_chg_cur(sprdbat_data->bat_info.chg_current_type);
                                        //sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_low_temp_area);
                                }
                                temp_modify_flag = 0x02;
                        }
			else if((sprdbat_data->bat_info.adp_type == ADP_TYPE_DCP) && ((sprdbat_data->bat_info.cur_temp <= sprdbat_data->pdata->high_temp_area_l_cur) &&
                        (sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->high_temp_area_l_cur_30))){
                                if(temp_modify_flag != 0x04){
                                        printk("battery temp in a low 40 area\n");
                                        sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_high_temp_area_30);
                                }
                                temp_modify_flag = 0x04;
                        }
                        else if((sprdbat_data->bat_info.adp_type == ADP_TYPE_DCP) && ((sprdbat_data->bat_info.cur_temp <= sprdbat_data->pdata->high_temp_area_h)&&
                                (sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->high_temp_area_l_cur))){
                                if(temp_modify_flag != 0x08){
                                        printk("battery temp in a high area\n");
                                        sprdbat_data->bat_info.temp_over_h = TEMP_OVER_45_LEVEL;
                                        sprdchg_set_chg_cur(sprdbat_data->pdata->chg_current_high_temp_area);

                                }
                                temp_modify_flag = 0x08;
				if(sprdbat_data->bat_info.cur_temp > sprdbat_data->pdata->high_temp_area_l){
                                if(sprdbat_data->bat_info.vbat_ocv > (sprdbat_data->pdata->chg_end_vol_high_temp)){
                                                sprdbat_data->bat_info.temp_over_h = TEMP_H_OVER_4100_OCV;
                                                sprdbat_change_module_state(SPRDBAT_CHG_OVERHEATLEVE_STOP_E);
                                }
                                printk("battery temp in a high area stop flag = 0x%x\n",sprdbat_data->bat_info.chg_stop_flags);
				if((sprdbat_data->bat_info.vbat_ocv < (sprdbat_data->pdata->chg_end_vol_high_temp-50)) &&
                                        sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_OVERHEAT_LEVE_BIT){
                                                temp_voltage_reset = 1;
                                                sprdbat_data->bat_info.temp_over_h = TEMP_H_BACK_4100_OCV;
                                                sprdbat_change_module_state(SPRDBAT_CHG_OVERHEATLEVE_RESET_E);
                                        }
                                printk("battery temp in normal area stop flag = 0x%x\n",sprdbat_data->bat_info.chg_stop_flags);
				}
				else{
					sprdbat_data->bat_info.temp_over_h = TEMP_BACK_45_LEVEL;
                                	if(sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_OVERHEAT_LEVE_BIT)
                                        	sprdbat_change_module_state(SPRDBAT_CHG_OVERHEATLEVE_RESET_E);
				}
                        }
			/*
                        else if(temp_modify_flag == 1){
                                printk("battery temp in normal area\n");
                                temp_modify_flag = 0;
...
static void sprdbat_charge_works(struct work_struct *work)
{
	...
	unsigned long irq_flag = 0;
	/*wdm---*/
	static uint32_t sprdbat_bad_chg=0;
        static uint32_t sprdbat_bad_current=0;
        static uint32_t sprdbat_ok_chg=0;
	/*---*/
	...
	if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT) {
		...
		if ((sprdbat_data->bat_info.vbat_vol >
                            ...
                            && (sprdbat_data->bat_info.bat_current < 200)) {
                                sprdbat_trickle_chg = 0;
                                SPRDBAT_DEBUG("0.1C SPRDBAT_CHG_FULL_E\n");
		...
		#if 0
		sprdbat_data->bat_info.vchg_vol = sprdchg_read_vchg_vol();
                     if(sprdbat_data->bat_info.vchg_vol  <4350)
                        {
                             sprdbat_bad_chg++;
                            printk
                                ("sprdbat_data->bat_info.vchg_vol:%d;bad:%d;current:%d;\n",
                                    sprdbat_data->bat_info.vchg_vol,sprdbat_bad_chg,sprdbat_bad_current);
                          if((sprdbat_bad_chg%2) ==0)
                                {
                                    if(sprdbat_bad_current == sprdbat_data->pdata->chg_max_cur)
                                        sprdbat_bad_current =   sprdbat_data->pdata->adp_dcp_cur;
                                    else if(sprdbat_bad_current == sprdbat_data->bat_info.chg_current_type)
                                       sprdbat_bad_current  = 400;
                                   else if( sprdbat_bad_current ==400 )
                                       sprdbat_bad_current  = 300;
                                   else
                                        {
                                        sprdbat_bad_current =0;
                                      sprdbat_change_module_state(SPRDBAT_ADP_PLUGOUT_E);
                                        }
                                    if(sprdbat_bad_current)
                                       sprdchg_set_chg_cur(sprdbat_bad_current);
                                  printk("sprdbat;vchg error : charger vol<4300!%d;\n",sprdbat_bad_current);
                                }
                        }
                        else
                        {
                           sprdbat_ok_chg++;
                          printk("sprdbat;vchg ok=%d; bad=%d;current=%d;\n",
                                                sprdbat_ok_chg,sprdbat_bad_chg,sprdbat_bad_current);
                          if(sprdbat_ok_chg ==5 && sprdbat_bad_chg == 0 && sprdbat_bad_current == sprdbat_data->pdata->adp_dcp_cur
                                        && (sprdbat_data->bat_info.cur_temp >150 && sprdbat_data->bat_info.cur_temp <450) )
                                {
                                      sprdbat_bad_current = sprdbat_data->pdata->chg_max_cur;
                                   sprdchg_set_chg_cur(sprdbat_bad_current);
                                  printk("sprdbat_data->bat_info.vchg_vol:%d;current:%d;\n",
                                         sprdbat_data->bat_info.vchg_vol,sprdbat_bad_current);
	...
	if(sprdbat_data->bat_info.module_state == POWER_SUPPLY_STATUS_FULL
                ...
                && sprdbat_data->bat_info.vbat_ocv < sprdbat_data->pdata->chg_end_to_chg_state_vol)
        {
                SPRDBAT_DEBUG("vbat_ocv =%d; <%d; module_state = %d;POWER_SUPPLY_STATUS_CHARGING;\n",
                        sprdbat_data->bat_info.vbat_ocv,
                        sprdbat_data->pdata->chg_end_to_chg_state_vol,
                        sprdbat_data->bat_info.module_state);
                sprdbat_data->bat_info.chg_this_timeout = sprdbat_data->pdata->chg_timeout;
							   ...
							   *pdev)
{
	...
	/*wdm---*/
	ret = of_property_read_u32(np, "chg-current-call",(u32 *) (&temp));
        pdata->chg_current_call = temp;
	ret = of_property_read_u32(np, "chg-current-high-temp-area",(u32 *) (&temp));
        pdata->chg_current_high_temp_area = temp;
	ret = of_property_read_u32(np, "chg-current-high-temp-area-30",(u32 *) (&temp));
        pdata->chg_current_high_temp_area_30 = temp;
        ret = of_property_read_u32(np, "chg-current-low-temp-area",(u32 *) (&temp));
        pdata->chg_current_low_temp_area = temp;
        ret = of_property_read_u32(np, "low-temp-area-h",(u32 *) (&temp));
        pdata->low_temp_area_h = temp - 1000;
        ret = of_property_read_u32(np, "low-temp-area-l",(u32 *) (&temp));
        pdata->low_temp_area_l = temp - 1000;
        ret = of_property_read_u32(np, "high-temp-area-h",(u32 *) (&temp));
        pdata->high_temp_area_h = temp - 1000;
        ret = of_property_read_u32(np, "high-temp-area-l",(u32 *) (&temp));
        pdata->high_temp_area_l = temp - 1000;
	/*wdm---*/
	ret = of_property_read_u32(np, "high-temp-area-l-cur",(u32 *) (&temp));
        pdata->high_temp_area_l_cur = temp - 1000;

	ret = of_property_read_u32(np, "high-temp-area-l-cur-30",(u32 *) (&temp));
        pdata->high_temp_area_l_cur_30 = temp - 1000;
	/*---*/

	ret = of_property_read_u32(np, "chg-end-vol-high-temp", &pdata->chg_end_vol_high_temp);
        ret = of_property_read_u32(np, "chg-end-to-chg-state-vol", &pdata->chg_end_to_chg_state_vol);
        ret = of_property_read_u32(np, "chg-max-cur", &pdata->chg_max_cur);
	/*---*/
	ret =
	...
	/*wdm---*/
	ret = of_property_read_u32(np, "discharging_rint", (u32 *) (&pdata->discharging_rint));
        ret = of_property_read_u32(np, "charging_rint", (u32 *) (&pdata->charging_rint));
	/*---*/
	...
	/*wdm---*/
	pdata->rint_tab_size =0;
        ret =
            of_property_read_u32(np, "rint-tab-size",
                                 (u32 *) (&pdata->rint_tab_size));

        SPRDBAT_DEBUG("sprdbat_parse_dt rint_tab_size=%d;%x;\n",pdata->rint_tab_size,ret);

       if(pdata->rint_tab_size)
       {
                pdata->rint_tab = kzalloc(sizeof(struct sprdbat_table_data) *
                                 pdata->rint_tab_size, GFP_KERNEL);

                for (i = 0; i < pdata->rint_tab_size; i++) {
                        ret =
                            of_property_read_u32_index(np, "rint-temp-tab", i,
                                                       (u32 *) (&temp));
                        pdata->rint_tab[i].x = temp - 1000;

                        ret = of_property_read_u32_index(np, "rint-tab", i,
                                                         (u32 *) (&pdata->rint_tab[i].y));
                }
       }
	/*---*/
	return pdata;
...
static int sprdbat_probe(struct platform_device *pdev)
{
    ...
    if(!charger_probe_ok)//add by liuwei
        charger_probe_ok=1;//add by liuwe

    if(!pmu_probe_ok)//add by liuwei
        pmu_probe_ok=1;//add by liuwe
