...
int mali_platform_device_init(struct platform_device *pdev)
{
	...
	/* A1000: границы диапазона задаём явно.
	 * of_property_read_u32 при отсутствии свойства не меняет переменную, а
	 * freq-range-max/min нет ни в одном DTS этого дерева — граница молча
	 * бралась от freq-default. Ставим осмысленные значения заранее. */
	i = gpu_dfs_ctx.freq_list_len - 1;
...
void mali_platform_utilization(struct mali_gpu_utilization_data *data)
{
	...
	{
		/* A1000: разгон по загрузке, а не пропорционально текущей частоте.
		 * Штатная формула target = freq_cur * load / 256 не даёт подняться:
		 * на низкой частоте GPU ждёт vsync и копирования кадра процессором,
		 * загрузка держится около 60-70%, до порога в 90% не дотягивает, а
		 * пропорция от низкой текущей частоты снова даёт низкую. В итоге
		 * интерфейс упирался в 20 FPS, хотя железо тянет втрое больше.
		 * Ставим обычный гистерезис: выше 70% — ступень вверх, ниже 30% —
		 * ступень вниз, между — держим. При выключенном экране и статичной
		 * картинке отрисовки нет, загрузка нулевая, частота уходит на
		 * минимум сама, так что экономия сохраняется. */
		const struct gpu_freq_info* cur = gpu_dfs_ctx.freq_cur;
		int target_freq;

		if (gpu_dfs_ctx.cur_load >= (256*5/10))	/* A1000: порог подъёма 50% */
		{
			gpu_dfs_ctx.freq_next = (cur < gpu_dfs_ctx.freq_max) ? (cur + 1) : gpu_dfs_ctx.freq_max;
			MALI_DEBUG_PRINT(3,("GPU_DFS util %3d; ступень вверх -> %6d\n",
				gpu_dfs_ctx.cur_load, gpu_dfs_ctx.freq_next->freq));
			goto a1000_chosen;
		}
		if (gpu_dfs_ctx.cur_load <= (256*2/10))	/* A1000: порог сброса 20% */
		{
			gpu_dfs_ctx.freq_next = (cur > gpu_dfs_ctx.freq_min) ? (cur - 1) : gpu_dfs_ctx.freq_min;
			MALI_DEBUG_PRINT(3,("GPU_DFS util %3d; ступень вниз -> %6d\n",
				gpu_dfs_ctx.cur_load, gpu_dfs_ctx.freq_next->freq));
			goto a1000_chosen;
		}

		target_freq = gpu_dfs_ctx.freq_cur->freq * gpu_dfs_ctx.cur_load / 256;
